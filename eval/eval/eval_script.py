...
def is_correct(item, pred_key='prediction', prec=1e-3):
    ...
    if isinstance(pred, list) and isinstance(ans, list):
        ...
        if '\\cup' in pred and '\\cup' in ans:
            ...
            return label
    elif isinstance(pred, list) and isinstance(ans, str):
        item_cpy = deepcopy(item)
        item_cpy.update({
            pred_key: pred,
            'answer': [ans]
        })
        return is_correct(item_cpy, pred_key=pred_key, prec=prec)
    else:
        print(item, flush=True)
        raise NotImplementedError()

def eval_olympiad_math(item, pred_key='prediction', prec=1e-3):
    pred = item[pred_key]
    ...
    flag = False
    try:
        pred = parse(pred[0] if len(pred) > 0 else "")
        ans = parse(ans)
        flag = verify(pred, ans)
    except:
        pass
    return flag
...
def eval_pubmedqa(item, pred_key='prediction', prec=1e-3):
    return eval_math_sat(item, pred_key=pred_key, prec=prec)
...
def eval_tabmwp(item, pred_key='prediction', prec=1e-3):
    if item['type'] == 'multiple-choice':
        return item[pred_key].lower() == item['answer'].lower()
    else:
        return eval_last_single_answer(item, pred_key=pred_key, prec=prec)

def eval_mathqa(item, pred_key='prediction', prec=1e-3):
    return eval_math_sat(item, pred_key=pred_key, prec=prec)